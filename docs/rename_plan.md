...
| --- | --- | --- | --- | --- | --- |
| cpp/cli/closeout_demo.cpp | cpp/cli/CLOSEOUT_DEMO.cpp | Main CLI entry point → ALL CAPS | Update root `CMakeLists.txt` source list entry for target `closeout_demo` to point to the new filename; adjust any docs/refs that mention the path | Medium | Follow Verification steps 1–3 below |
