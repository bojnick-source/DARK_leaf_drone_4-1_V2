...
lift::Design LegacyDiskAreaAdapter::translate_to_legacy(const RotorGeometry& geometry) const {
    ...
    // Create minimal Design object for legacy computation
    lift::Design design;
    ...
    // Set reasonable defaults for other required Design fields
    // (not used by disk area calculation but required for validation)
    design.arch = lift::Architecture::Multicopter_Open;
    design.rotor_rpm = 1000.0;       // arbitrary, not used
    design.rotor_solidity = 0.1;     // arbitrary, not used
    design.coaxial_spacing_m = 0.2;  // arbitrary, only used if is_coaxial
    
    // Set minimal mass to pass validation (not used by disk area calc)
