/*
================================================================================
Legacy 4.1 — Analysis: Closeout Evaluation (GO/NO-GO Gate Engine)
FILE: engine/analysis/closeout_eval.cpp
================================================================================
*/
...
GateResult evaluate_gates(const CloseoutReport& r, const CloseoutEvalOptions& opt) {
  ...
  if (is_set(r.gates.max_delta_mass_kg)) {
    ...
    } else if (r.mass_delta.delta_mass_total_kg <= r.gates.max_delta_mass_kg) {
      mark_pass();
    } else {
  ...
  if (is_set(r.gates.min_A_total_m2)) {
    ...
    } else if (r.disk.A_total_m2 >= r.gates.min_A_total_m2) {
      mark_pass();
    } else {
      std::ostringstream ss;
  ...
  if (is_set(r.gates.min_yaw_margin_ratio)) {
    ...
    } else if (r.maneuver.authority.yaw_margin_ratio >= r.gates.min_yaw_margin_ratio) {
      mark_pass();
    } else {
      std::ostringstream ss;
  ...
  if (is_set(r.gates.min_phase_tolerance_deg)) {
    ...
    } else if (r.sync_risk.phase_tolerance_deg >= r.gates.min_phase_tolerance_deg) {
      mark_pass();
    } else {
      std::ostringstream ss;
  ...
  if (is_set(r.gates.max_latency_ms)) {
    ...
    } else if (r.sync_risk.estimated_latency_ms <= r.gates.max_latency_ms) {
      mark_pass();
    } else {
      std::ostringstream ss;
