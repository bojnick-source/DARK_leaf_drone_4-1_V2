...
| CORE-02 | No silent guessing: derive values deterministically or fail with clear error | All modules under `src/sfcs_mdp/` | `python -m pytest` | 139 tests pass | PASS |
...
| CORE-07 | Security: default offline, no geometry upload without explicit flag | N/A (no online features) | Manual review | No network calls in codebase | PASS |
